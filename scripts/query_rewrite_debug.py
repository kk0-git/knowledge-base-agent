...
from services.rag.query_rewrite import LLMQueryRewriter, QueryRewriteParseError


def main() -> int:
    parser = argparse.ArgumentParser(description="Generate standalone LLM query rewrites for debugging")
    parser.add_argument("--query", default=None, help="Single query to rewrite")
    parser.add_argument("--input", default=None, help="JSON file containing query cases")
    parser.add_argument("--out", default="./eval-results/query-rewrite-debug.json", help="Output JSON path")
    ...
    for index, case in enumerate(cases, start=1):
        ...
        try:
            ...
            item = {
                ...
                "ok": True,
                "input_case": case,
                "rewrite": asdict(result.rewrite),
                "validation_warnings": result.validation_warnings,
                "raw_output": result.raw_output,
                "error": None,
            }
        except QueryRewriteParseError as exc:
            item = {
                "query": query,
                "ok": False,
                "input_case": case,
    ...
    print("")
    print(f"Saved query rewrite debug result: {out_path.resolve()}")
    print(
        "Summary: "
