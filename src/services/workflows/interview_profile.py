...
class InterviewProfileStore:
    ...
    def save(self, profile: dict[str, Any]) -> None:
        ...
        if int(profile.get("schema_version") or 0) >= 4 and profile.get("beliefs") is not None:
    ...
    def apply_user_commit(
        ...
        *,
        ...
        collection = "procedures" if target == "procedure" else "beliefs"
        target_before = next(
            (item for item in model_before.get(collection) or [] if str(item.get("id") or "") == target_id),
            None,
        )
    ...
    def list_candidate_procedures(self) -> list[dict[str, Any]]:
        ...
        return [
            dict(procedure)
            for procedure in model.get("procedures") or []
