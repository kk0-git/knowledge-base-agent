    ...
    normalized.update(
        {
            ...
            "category": normalize_category(belief.get("category")),
...
def normalize_learner_model(model: Any) -> dict[str, Any]:
    ...
    normalized["beliefs"] = [normalize_belief(item) for item in ensure_list(normalized.get("beliefs"))]
    normalized["procedures"] = [normalize_procedure(item) for item in ensure_list(normalized.get("procedures"))]
