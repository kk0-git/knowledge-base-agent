...
def beliefs_to_weak_points(model: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for belief in model.get("beliefs") or []:
...
def sync_profile_view_to_model(profile: dict[str, Any], model: dict[str, Any]) -> dict[str, Any]:
    ...
    beliefs_by_id = {str(item.get("id") or ""): item for item in updated.get("beliefs") or [] if isinstance(item, dict)}
            ...
            mapped.append(
                {
                    ...
                    "planned_layer": prepared.get("planned_layer") or "",
                    "category": prepared.get("category") or "knowledge_gap",
                    "scope": prepared.get("scope") or normalize_scope(
                        prepared.get("scope_suggestion"),
                        category=prepared.get("category") or "knowledge_gap",
                    ),
                    "domain_anchor": prepared.get("domain_anchor") or {},
                    "session_id": session_id,
    ...
    *,
    ...
    for raw in observations or []:
        ...
        if obs_type == "partial":
            ...
            legacy_ops = {"strong_added": [], "strong_updated": []}
            upsert_strong_point(
                profile_view,
                raw,
                session_id=session_id,
            ...
            updated["strong_points"] = list(profile_view.get("strong_points") or [])
            operations["strong_added"].extend(legacy_ops.get("strong_added") or [])
            operations["strong_updated"].extend(legacy_ops.get("strong_updated") or [])
