...
def _commit_propose_belief(
    ...
    new_belief = _belief_from_observation(observation, today)
    force_new_candidate = bool(observation.get("force_new_candidate")) and new_belief.get("lifecycle") == "candidate"
    target = None if force_new_candidate else _find_merge_target(model.get("beliefs", []), new_belief)
    changed = False
    if target:
        ...
        model.setdefault("beliefs", []).append(new_belief)
...
def _belief_from_observation(observation: dict[str, Any], today: str) -> dict[str, Any]:
    ...
    lifecycle = _default_lifecycle(source_kind, confidence, target_lifecycle)
...
def _commit_propose_procedure(
    ...
    if not new_procedure.get("title") and not new_procedure.get("steps"):
        return False
    target = _find_procedure_merge_target(model.get("procedures", []), new_procedure)
    if target:
        changed = _merge_into_procedure(target, new_procedure, today)
        operations["updated_procedures"] += 1
        return changed
    model.setdefault("procedures", []).append(new_procedure)
...
def _find_merge_target(beliefs: list[dict[str, Any]], new_belief: dict[str, Any]) -> dict[str, Any] | None:
    ...
    for belief in beliefs:
        ...
        if belief.get("scope") != new_belief.get("scope"):
            continue
        if belief.get("category") != new_belief.get("category"):
            continue
