...
_CATEGORY_PROBE_HINTS: dict[str, str] = {
    "knowledge_gap": "probe definition and underlying mechanism",
    "answer_structure": "probe how they organize and frame the answer",
    "communication": "probe clarity and precision of explanation",
    "depth": "probe implementation detail and tradeoffs",
    "recall": "probe whether they can retrieve the concept under pressure",
}
...
def render_librarian_memory_context(
    *,
    ...
    selected = _select_beliefs_from_ranked(ranked, max_beliefs=3, max_due_markers=2)

    procedures = _active_procedures(model, limit=2)
    derived = model.get("derived") or {}
    blurb = _pick_derived_blurb(derived, scope_value, scope_note_paths)

    if not selected and not procedures and not blurb:
        return ""

    lines = [
        "## Learner Memory Background",
        "Private background from prior sessions. Use quietly; do not mention memory, weak points, or review history unless the user asks.",
        "",
    ]
    if blurb:
        lines.extend([blurb, ""])
    ...
    lines.extend(
        [
            "## Memory Use Boundary",
            "Let the user's current question and note evidence drive the answer.",
...
def render_interviewer_memory_context(
    *,
    ...
    selected = _select_beliefs_from_ranked(ranked, max_beliefs=5, max_due_markers=None)

    procedures = _active_procedures(model, limit=2)
    derived = model.get("derived") or {}
    blurb = _pick_derived_blurb(derived, scope_value, scope_note_paths)
    commitments = _commitments_for_prompt(model)
    ...
    lines = [
        "## Learner Memory Background",
        "Private background from prior sessions. Use quietly to shape probes; do not mention memory, weak points, or review history unless the user asks.",
        "",
    ]
    if blurb:
        lines.extend(["### Derived domain summary", blurb, ""])
    if selected:
    ...
    lines.extend(
        [
            "## Memory Use Boundary",
            "Let the current layer objective and the user's live answer drive the next probe.",
...
def _active_procedures(model: dict[str, Any], *, limit: int) -> list[dict[str, Any]]:
    return [
        item
        for item in model.get("procedures") or []
...
def _probe_hint_for_weak(weak: dict[str, Any]) -> str:
    ...
    if explicit:
        return explicit
    category = str(weak.get("category") or "").strip()
    if category in _CATEGORY_PROBE_HINTS:
        return _CATEGORY_PROBE_HINTS[category]
    return f"probe {category}" if category else "probe for signal on this layer"
