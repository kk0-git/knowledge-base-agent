...
def test_normalize_v4_input_preserves_unknown_fields():
    ...
    assert normalized["beliefs"][0]["custom_belief_field"] == 123
    assert normalized["beliefs"][0]["lifecycle"] == "candidate"
...
def test_store_loads_legacy_in_memory_and_save_writes_only_v4(tmp_path):
    ...
    assert model["schema_version"] == 4
    assert model["beliefs"][0]["point"] == "Needs clearer answer structure"

    model["beliefs"][0]["point"] = "Updated canonical point"
    store.save(model)

    assert v4_path.exists()
    saved = json.loads(v4_path.read_text(encoding="utf-8"))
    legacy = json.loads(legacy_path.read_text(encoding="utf-8"))
    assert saved["beliefs"][0]["point"] == "Updated canonical point"
