...
def test_propose_belief_default_lifecycle_rules():
    ...
    assert [belief["lifecycle"] for belief in model["beliefs"]] == ["active", "candidate", "active"]
...
def test_standard_belief_merges_when_scope_domain_category_and_anchor_match():
    ...
    assert model["beliefs"][0]["times_seen"] == 2
    assert len(model["beliefs"][0]["evidence_refs"]) == 2
...
def test_archived_belief_does_not_merge_or_revive():
    ...
    assert model["beliefs"][0]["lifecycle"] == "archived"
    assert model["beliefs"][1]["lifecycle"] == "candidate"
...
def test_confusion_pair_merges_by_unordered_key():
    ...
    assert len(model["beliefs"]) == 1
    assert model["beliefs"][0]["distinction"] == "Semantic vs keyword."
    assert model["beliefs"][0]["times_seen"] == 2
...
def test_improvement_only_updates_active_belief_and_adds_commitment():
    model = default_learner_model()
    model["beliefs"].append({"id": "wp-1", "lifecycle": "active", "point": "Needs practice"})
    model["beliefs"].append({"id": "wp-2", "lifecycle": "candidate", "point": "Candidate only"})
    ...
    assert model["beliefs"][0]["improved"] is True
    assert model["beliefs"][1].get("improved") is False
...
def test_user_commit_confirm_deny_and_restore():
    ...
    assert [belief["lifecycle"] for belief in model["beliefs"]] == ["active", "archived", "active"]
...
def test_propose_procedure_defaults_candidate_and_merges_by_key():
    ...
    assert model["procedures"][0]["lifecycle"] == "candidate"
    assert model["procedures"][0]["procedure_key"] == "answer_format.conclusion_first"
...
def test_propose_procedure_merges_by_title_similarity():
    ...
    assert len(model["procedures"]) == 1
    assert model["procedures"][0]["times_seen"] == 2


def test_user_commit_confirm_deny_and_restore_procedure():
    model = default_learner_model()
    model["procedures"].extend(
        [
    ...
    assert [procedure["lifecycle"] for procedure in model["procedures"]] == ["active", "archived", "active"]
    assert ops["commitments_added"] == 3
    assert ops["archived_procedures"] == 1


def test_interview_candidate_merges_to_active_when_answer_medium_matches():
    model = default_learner_model()
    model, _ = commit_observations(
        model,
        [
    ...
    assert model["beliefs"][0]["lifecycle"] == "candidate"
    ...
    assert len(model["beliefs"]) == 1
    assert model["beliefs"][0]["lifecycle"] == "active"
    assert "interview" in model["beliefs"][0]["source_kinds"]
    assert "answer" in model["beliefs"][0]["source_kinds"]
